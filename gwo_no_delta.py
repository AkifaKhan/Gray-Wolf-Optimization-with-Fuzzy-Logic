"""
Standard GWO-based Decision Tree optimizer (NO-DELTA version)

- Delta (third-best wolf) layer removed from the update rule.
- Only Alpha and Beta wolves influence the position updates.

Optimizes DecisionTree hyperparameters using the original Grey Wolf Optimizer variant.
Uses a simple linear schedule for exploration weight `a = 2 - 2*(iteration/max_iter)`.

Expected input file:
    sepsis_data_encoded.csv  (encoded or numeric dataset with binary target 'inhospital_mortality')
...
def eval_fitness(position, X, y):
    ...
    try:
        ...
        X_tr, X_val, y_tr, y_val = train_test_split(X, y, test_size=0.3, stratify=y, random_state=77)
        clf = DecisionTreeClassifier(
            max_depth=max_depth,
            min_samples_split=min_samples_split,
            min_samples_leaf=min_samples_leaf,
    ...
    for t in range(1, max_iter + 1):
        ...
        for i in range(n_agents):
            for d in range(dim):
                ...
                # Update (NO DELTA layer)
                positions[i, d] = (X1 + X2) / 2.0
...
if __name__ == "__main__":
    ...
    with open("model_results.csv", "a", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["GWO-DT-no-delta", acc, prec, rec, f1])
