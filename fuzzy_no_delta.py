"""
Fuzzy-GWO optimized Decision Tree (fuzzy_gwo_dt.py) — ENHANCED PERFORMANCE VERSION (NO DELTA LAYER)

Key Improvements:
 - Removed Delta layer (optimization now guided only by Alpha & Beta wolves)
 - Stronger and adaptive fuzzy influence: weight between fuzzy_a and linear_a depends on diversity.
 - Lévy flight injection is adaptive (bigger when diversity is very low).
...
# ---------------- Main Fuzzy GWO (No Delta Layer) ----------------
def fuzzy_gwo_optimize(X, y, bounds, n_agents=20, max_iter=100, seed=42,
    ...
    alpha_pos, beta_pos = positions[sorted_idx[:2]].copy()
    alpha_score, beta_score = fitness[sorted_idx[:2]]
    ...
    for t in range(1, max_iter + 1):
        ...
        smooth_factor = 0.3 + 0.4 * (1 - diversity_norm)
        ...
        # Inject diversity via Lévy flight
        ...
        for i in range(n_agents):
            for d in range(dim):
                ...
                positions[i, d] = (
                    0.7 * positions[i, d] + 0.3 * (X1 + X2) / 2.0
...
if __name__ == "__main__":
    ...
    best_pos, best_score, history = fuzzy_gwo_optimize(X, y, bounds, n_agents=20, max_iter=100)
    ...
    for tr_idx, val_idx in skf.split(X, y):
        clf = DecisionTreeClassifier(max_depth=bd, min_samples_split=bss,
                                     min_samples_leaf=bsl, max_features=bf, random_state=42)
    ...
    clf = DecisionTreeClassifier(max_depth=bd, min_samples_split=bss, min_samples_leaf=bsl,
                                 max_features=bf, random_state=42)
    ...
    with open("model_results.csv", "a", newline="") as f:
        csv.writer(f).writerow(["Fuzzy-GWO-DT (No Delta)", acc, prec, rec, f1])
